...
enum Rank
{ TWO =
    2, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, TEN, JACK, QUEEN, KING,
    ACE };
enum Suit
...
Hand::isThreeOfKind ()
{
  if (cards[0].rank_ == cards[1].rank_
      ...
      && cards[2].rank_ != cards[3].rank_ && cards[3].rank_ != cards[4].rank_)
    return 1;
  if (cards[0].rank_ != cards[1].rank_
      && cards[1].rank_ == cards[2].rank_
      && cards[2].rank_ == cards[3].rank_ && cards[3].rank_ != cards[4].rank_)
...
Hand::isFlush ()
{
  for (int i = 1; i < size; i++)
    if (cards[i].suit_ != cards[0].suit_)
      return 0;
  return 1;
}
...
void
Deck::deal (Hand & hand, unsigned size)
{
